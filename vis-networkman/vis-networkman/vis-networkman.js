...
VisNetworkMan.prototype.renderWithDataList = function(dataList, option){
    ...
    (this.globalOption.extendsOption.funcEventSetupWhenFinishRender && this.globalOption.extendsOption.funcEventSetupWhenFinishRender(this));
    return container;
};
...
VisNetworkMan.prototype.setupEvent = function(network, extendsOption){
    ...
    network.on('click', function(e){
        ...
        var nodeMap = nodes._data;
        //- Run Node
        for (var i=0, nodeId, node; i<e.nodes.length; i++){
            nodeId = e.nodes[i];
            node = nodeMap[nodeId];
            (extendsOption.funcEventClickNode && extendsOption.funcEventClickNode(node));
        }
    });
    network.on('doubleClick', function(e){
        ...
        if (e.nodes){
            ...
            for (var i=0, nodeId, node; i<e.nodes.length; i++){
                nodeId = e.nodes[i];
                node = nodeMap[nodeId];
                (extendsOption.funcEventDoubleClickNode && extendsOption.funcEventDoubleClickNode(node));
            }
        }
    });
