 ...
 *
 ***************************************************************************/
JellySpeaker.PluginAlarmManager = function(){
    this.name = '';
    this.parent = null;
    this.statusSetuped = false;
};
JellySpeaker.PluginAlarmManager.SYSTEM_ICON_ID_TO_OPEN = 'open-plugin-alarm-manager';
JellySpeaker.PluginAlarmManager.USER_ICON_ID_TO_OPEN = 'open-alarm-manager';
JellySpeaker.PluginAlarmManager.POP_MANAGER = 'pop-alarm-manager';



JellySpeaker.PluginAlarmManager.prototype.setup = function(parent){
    var that = this;
    ...
    parent.setupSystemIcon([
        new JellySpeaker.Icon(JellySpeaker.PluginAlarmManager.SYSTEM_ICON_ID_TO_OPEN).setCommand(function(){
            popman.pop(JellySpeaker.PluginAlarmManager.POP_MANAGER);
        })
    ]);
    /** User Icon **/
    parent.setupUserIcon([
        new JellySpeaker.Icon(JellySpeaker.PluginAlarmManager.USER_ICON_ID_TO_OPEN).setTitle('알람 설정').setHTML('⏰').setCommand(JellySpeaker.PluginAlarmManager.SYSTEM_ICON_ID_TO_OPEN).setRunner([
        ]),
    ]);
    /** Make Component **/
    ready(function(){
...
JellySpeaker.PluginAlarmManager.prototype.start = function(){

};
JellySpeaker.PluginAlarmManager.prototype.end = function(){

};
JellySpeaker.PluginAlarmManager.prototype.dispose = function(){
 ...
 *
 **************************************************/
JellySpeaker.PluginAlarmManager.prototype.makePopForAlarmManager = function(){
    var that = this;
    var parent = this.parent;
    var boxman = parent.boxman;
    var popman = parent.popman;
    ...
    var popElementForManager = popman.add(popman.new({
        id: JellySpeaker.PluginAlarmManager.POP_MANAGER,
        exp: '95%',
        closebyesc: true,
